...
int main() {
    ...
    for (int i = 1; i <= 5; ++i) {         //@ @guide 目前的 vector：{v} @tts [next] 準備一個一個放進元素 | @2 [next] 繼續 push_back，注意容量虛線框什麼時候才變大
        v.push_back(i * 10);               //@ @guide push_back 之後：{v} @tts [next] 放進第 {i} 個元素，格子多一個
    }
