...
int main() {
    std::vector<int> v = {3, 1, 4, 1, 5};  //@ @guide 資料：{v} @tts [next] 這是等一下要處理的五個數字 @layout sidebar:45 open:container
    int sum = 0;                           //@ @guide sum 初始化為 {sum} @tts [next] 累加器歸零。第一題就綁在這一行
    for (int i = 0; i < 5; ++i) {          //@ @guide 累加到第 {i} 個，目前 sum = {sum} @tts [next] 開始累加 | @2 [next] 一個一個加進去
        sum = sum + v[i];                  //@ @guide sum = {sum} @tts [next] 加上 {v} 的第 {i} 格
    }
    ...
    for (int j = 1; j < 5; ++j) {          //@ @guide 找最大值，目前 mx = {mx} @tts [next] 換一輪，這次找最大的 | @2 [next] 比較下一個
        if (v[j] > mx) {                   //@ @guide 比較 v[{j}] 和目前的 mx = {mx} @tts [next] 這一個比較大嗎 | @2 [next] 再比一次
            mx = v[j];                     //@ @guide 更新 mx = {mx} @tts [next] 比較大，換它當最大值 | @2 [next] 又找到更大的
        }
    }
