...
int main() {
    ...
    for (int i = 1; i <= h; ++i) {               //@ @guide 外層迴圈選一列：現在填第 {i} 列（對應地圖第 {i} - 1 列）\n{dp} @tts [next] 外層迴圈一列一列往下 | @2 [next] 這一列填完了，換第 {i} 列 @layout sidebar:55 open:container
        for (int j = 1; j <= w; ++j) {           //@ @guide 內層迴圈選一行：第 {i} 列第 {j} 行\n{dp[i][j]:lightblue} @tts [next] 內層迴圈由左往右走完這一列 | @2 [next] 往右一格，現在是第 {j} 行
            if (g[i - 1][j - 1] == '#') {        //@ @guide 先看這一格是不是牆（地圖上的 # 就是牆） @tts [next] 先問這一格是不是牆 | @2 [next] 是牆嗎
            ...
            int up = dp[i - 1][j];               //@ @guide 從上面來的走法有幾種\n{dp[i - 1][j]:orange} @tts [next] 能走到這一格的路只有兩種來源。先看從上面來的 | @2 [next] 從上面來的
