...
int main() {
    ...
    for (int i = 1; i <= h; ++i) {               //@ @guide 外層迴圈：一列一列往下填\n{dp} @tts [next] 外層迴圈一列一列往下填 | @2 [next] 這一列填完了，看還有沒有下一列 @layout sidebar:55 open:container
        for (int j = 1; j <= w; ++j) {           //@ @guide 現在填第 {i} 列（對應地圖第 {i} - 1 列）\n內層迴圈由左往右走完這一列\n{dp} @tts [next] 現在填第 {i} 列，內層迴圈由左往右走完這一列 | @2 [next] 第 {i} 列，看還有沒有下一格
            if (g[i - 1][j - 1] == '#') {        //@ @guide 第 {i} 列第 {j} 行\n{dp[i][j]:lightblue} 這一格\n{dp[i - 1][j]:orange} 上面\n{dp[i][j - 1]:lime} 左邊\n先看這一格是不是牆（地圖上的 # 就是牆） @tts [next] 第 {i} 列第 {j} 行。橘色是上面，綠色是左邊。先問這一格是不是牆 | @2 [next] 第 {i} 列第 {j} 行，是牆嗎
            ...
            int up = dp[i - 1][j];               //@ @guide 不是牆。能走到這一格的路有兩種來源，先讀上面那格\n{dp[i - 1][j]:orange} 上面\n{dp[i][j - 1]:lime} 左邊\n{dp[i][j]:lightblue} 這一格 @tts [next] 能走到這一格的路只有兩種來源。先看從上面來的 | @2 [next] 從上面來的
            int left = dp[i][j - 1];             //@ @guide 上面來的有 {up} 種\n{dp[i - 1][j]:orange} 上面\n{dp[i][j - 1]:lime} 再讀左邊\n{dp[i][j]:lightblue} 這一格 @tts [next] 上面來的有 {up} 種。再看從左邊來的 | @2 [next] 再看左邊
