...
int main() {
    ...
    for (int i = 1; i <= n; ++i) {                                              //@ @guide 外層迴圈選一列：一件一件物品加進來考慮\n{dp} @tts [next] 外層迴圈每跑一圈就多考慮一件物品 | @2 [next] 這一列填完了，看還有沒有下一件 @layout sidebar:58 open:container
        int wi = weights[i - 1];                                                //@ @guide 現在把第 {i} 件物品加進來考慮\n第 {i} 件的重量\n{weights[i - 1]:orange} @tts [next] 現在考慮第 {i} 件。先把這一件的重量拿出來 | @2 [next] 第 {i} 件，換這一件的重量
        int vi = values[i - 1];                                                 //@ @guide 第 {i} 件：重量 {wi}\n它的價值\n{values[i - 1]:orange} @tts [next] 再把它的價值拿出來，這一件重 {wi} | @2 [next] 這一件重 {wi}
        for (int w = 0; w <= capacity; ++w) {                                   //@ @guide 內層迴圈選一行：第 {i} 列，把容量上限一路往右試\n這一件重 {wi}、值 {vi}\n{dp} @tts [next] 內層迴圈把容量上限從零一路試到六 | @2 [next] 這一格算完了，看容量上限還能不能再往右
            int skip = dp[i - 1][w];                                            //@ @guide 現在容量上限是 {w}\n不拿第 {i} 件：答案就是上一列同一行\n{dp[i - 1][w]:pink} 抄到 {dp[i][w]:lightblue} @tts [next] 現在容量上限是 {w}。先看不拿這一件。不拿的話，能拿的價值就跟上一列同一行一樣 | @2 [next] 容量上限 {w}，不拿的話，抄上面那一格
