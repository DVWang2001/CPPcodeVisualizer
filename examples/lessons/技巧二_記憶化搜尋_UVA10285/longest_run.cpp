...
int main() {
    std::cin >> h >> w;                          //@ @guide 先讀地圖的大小：h 列、w 欄 @tts [next] 先讀進地圖有幾列、幾欄 @layout sidebar:55 open:container close:locals
    ...
    for (int i = 0; i < h; ++i) {                //@ @guide 一列一列把數值讀進來 @tts [next] 一列一列把每一格的數值讀進來 | @2 [next] 讀完一列，看還有沒有下一列
        for (int j = 0; j < w; ++j) {            //@ @guide 第 {i} 列，由左往右讀 @tts [next] 這一列由左往右讀 | @2 [next] 看還有沒有下一格
            std::cin >> a[i][j];                 //@ @guide 讀第 {i} 列第 {j} 欄\n{a} @tts [next] 讀第 {i} 列第 {j} 欄 | @2 [next] 讀第 {i} 列第 {j} 欄 | @3 [fast @9] 其他格子都是同一個動作，直接跳到全部讀完 | @9 [next] 九格都讀完了
        }
    }
