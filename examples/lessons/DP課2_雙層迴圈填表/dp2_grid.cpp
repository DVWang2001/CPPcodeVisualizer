...
int main() {
    ...
    for (int i = 1; i <= 3; ++i) {          //@ @guide 外層迴圈選一列\n{table} @tts [next] 外層迴圈負責選列，從第一列開始 | @2 [next] 這一列填完了，看還有沒有下一列 @layout sidebar:58 open:container
        for (int w = 0; w <= 6; ++w) {      //@ @guide 現在要填第 {i} 列\n內層迴圈選一行，由左往右走完這一列\n{table} @tts [next] 現在填第 {i} 列，內層迴圈負責從左到右走完這一列 | @2 [next] 第 {i} 列，看還有沒有下一格
            int value = i * 10 + w;         //@ @guide 第 {i} 列，第 {w} 行\n{table[i][w]:lightblue}\n這一格要填的值，只由 {i} 和 {w} 決定 @tts [next] 現在是第 {i} 列第 {w} 行。這一格填的是 i 乘以十再加 w | @2 [next] 由 {i} 和 {w} 算出這一格的值
