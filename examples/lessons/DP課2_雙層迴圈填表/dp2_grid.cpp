...
int main() {
    std::vector<std::vector<int>> table(4, std::vector<int>(7, 0));  //@ @guide 四列七行，先全部填 0：{table}\n第 0 列是邊界，等一下不會去動它 @tts [next] 這次是一張四列七行的表，先全部填零。第零列留著不動 @layout sidebar:58 open:container close:locals
