...
- **暫時的計算結果**：這一行通常是斷點停在賦值「之前」，GDB 還沒真的把新值寫進目標格，畫面上還是舊值。`pull:` 會把兩個來源格目前的數值加起來，用虛線框浮在目標格上方當作暫時結果，等真的停到下一行、GDB 給出目標格的實際值，這個暫時結果就會消失，換成正常顯示。目前只支援**兩數相加**（兩邊都要能轉成數字）；不是加法或任一邊不是數字的話，就只有飛行動畫，沒有暫時結果。
