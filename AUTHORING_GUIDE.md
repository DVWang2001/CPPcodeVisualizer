...
- **讀資料的行不需要指導欄。** `std::cin >> ...`、`getline`、`scanf` 這類把輸入讀進
  變數的行，一律不寫 `@guide`：讀進來的值學生在 Standard Input 面板就看得到，這一行
  的重點是「讀了幾個、放進哪個變數」，一句 `@tts` 講完就夠，不必再用指導欄重複一次
  或顯示容器。**只拿掉 `@guide`，`@tts` 和 `@layout` 照留**——§1.3 的硬性標準要求
  每個停駐點都有 `@tts` 自動播放指令，`@layout`（例如第一行讀完尺寸後展開容器面板）
  也是另一回事。注意這條只管「讀」的那一行；讀完之後開陣列、算尺寸的宣告行照常寫。

  ```cpp
  std::cin >> h >> w;      //@ @tts [next] 先讀進矩陣有幾列、幾欄 @layout sidebar:55 open:container
  std::cin >> cost[i][j];  //@ @tts [next] 讀第 {i} 列第 {j} 欄的成本 | @2 [next] ... | @3 [fast @20] ...
  ```
