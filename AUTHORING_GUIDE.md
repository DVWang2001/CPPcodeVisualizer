...
- **暫時的計算結果**：這一行通常是斷點停在賦值「之前」，GDB 還沒真的把新值寫進目標格，畫面上還是舊值。`pull:` 會把兩個來源格目前的數值加起來，用虛線框浮在目標格上方當作暫時結果——**這個暫時結果不是限時消失**，是持續蓋著，直到輪詢偵測到目標格的真實值真的變了（使用者步進到下一行、GDB 真的執行了那行賦值）才收掉，換成正常顯示。這樣才不會發生「合併完又變回舊值」的閃爍——如果是限時收掉，時間一到、GDB 還沒真的賦值，底下露出來的就還是舊值，看起來像動畫失敗。目前只支援**兩數相加**（兩邊都要能轉成數字）；不是加法或任一邊不是數字的話，就只有飛行動畫，沒有暫時結果。
