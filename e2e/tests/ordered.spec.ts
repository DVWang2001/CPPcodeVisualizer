...
import { runToBreakpoint, enableBSTMode } from './helpers';
...
test('set: cells show 3, 5, 7 (sorted order)', async ({ page }) => {
...
test('set: BST mode renders SVG tree with 3 nodes', async ({ page }) => {
    await enableBSTMode(page, 'se');
    const nodes = page.locator('[data-testid="container-se"] [data-testid="bst-node"]');
    await expect(nodes).toHaveCount(3);
});

test('set: BST nodes carry correct data-key attributes', async ({ page }) => {
    await enableBSTMode(page, 'se');
...
test('multiset: cells show 2, 2, 4 (sorted, duplicates allowed)', async ({ page }) => {
...
test('multiset: BST mode renders SVG tree with 3 nodes', async ({ page }) => {
...
test('map: rows show key=1 value=a and key=2 value=b', async ({ page }) => {
...
test('map: BST mode renders SVG tree with 2 nodes', async ({ page }) => {
...
test('multimap: rows show both key=1 entries plus their values', async ({ page }) => {
...
test('multimap: BST mode renders SVG tree', async ({ page }) => {
